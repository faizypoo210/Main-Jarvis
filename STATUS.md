...
| **GitHub — governed create issue** | **Implemented** (narrow) | `POST /api/v1/missions/{id}/integrations/github/create-issue`, `app/services/github_issue_workflow.py`, `app/integrations/github_adapter.py` | Approval-gated (`github_create_issue`, risk **red**); `JARVIS_GITHUB_TOKEN` on control plane; receipts `github_issue_created` / `github_issue_failed` | `docs/INTEGRATIONS_GITHUB.md`; approve then check mission timeline + Activity |
