...
| **Gmail — governed create draft** | **Implemented** (narrow) | `POST /api/v1/missions/{id}/integrations/gmail/create-draft`, `app/services/gmail_draft_workflow.py`, `app/integrations/gmail_adapter.py` | Approval-gated (`gmail_create_draft`, risk **red**); OAuth on control plane (`JARVIS_GMAIL_*`); **draft only**, no send | `docs/INTEGRATIONS_GMAIL.md` |
