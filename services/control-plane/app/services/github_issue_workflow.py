"""Governed GitHub issue creation: approval-gated, receipt-backed."""
...
from app.integrations.github_adapter import create_issue
...
from app.schemas.github_issue import GitHubCreateIssueContract, GitHubCreateIssueRequest, GitHubIssueResult
...
def human_reason(contract: GitHubCreateIssueContract) -> str:
    return f"Create GitHub issue in {contract.repo}: {contract.title}"


def contract_to_command_text(contract: GitHubCreateIssueContract) -> str:
    return contract.model_dump_json()


async def submit_create_issue_request(
    session: AsyncSession,
    *,
    mission_id: UUID,
    body: GitHubCreateIssueRequest,
) -> Approval:
    """Create approval (with structured command_text) + integration_action_requested."""
    from fastapi import HTTPException, status

    from app.services.approval_service import ApprovalService

    contract = GitHubCreateIssueContract.model_validate(
    ...
    await MissionEventRepository.create(
        ...
        actor_type="system",
        actor_id="github_issue_workflow",
...
async def execute_github_issue_after_approval(
    session: AsyncSession,
    approval: Approval,
) -> None:
    """After operator approves: call GitHub, record receipts + events, set mission terminal status."""
    ...
    try:
        contract = GitHubCreateIssueContract.model_validate_json(raw)
        ...
        await _failure_path(
            ...
            GitHubIssueResult(
                ...
                error_message=(
                    ...
                    "Set a GitHub fine-grained or classic PAT with issues:write for the target repo."
                ),
            ),
        )
        await session.commit()
    ...
    result = await create_issue(token=token, contract=contract)
    if result.success:
...
def _receipt_payload(result: GitHubIssueResult) -> dict[str, Any]:
...
async def _success_path(
    ...
    summary = (
        f"Issue #{result.issue_number} in {result.repo}"
        if result.issue_number is not None
        else f"Issue created in {result.repo}"
    )
    await receipts.record_receipt(
        mission_id=mission_id,
        receipt_type="github_issue_created",
    ...
    log.info(
        "github issue created mission_id=%s repo=%s number=%s",
        mission_id,
        result.repo,
        result.issue_number,
    )
...
async def _failure_path(
    ...
    missions: MissionRepository,
    mission_id: UUID,
    contract: GitHubCreateIssueContract | None,
    result: GitHubIssueResult,
) -> None:
    payload = _receipt_payload(result)
    repo = result.repo or (contract.repo if contract else "")
    title = result.title or (contract.title if contract else "")
    summary = result.error_message or "GitHub issue creation failed"
    await receipts.record_receipt(
        mission_id=mission_id,
        receipt_type="github_issue_failed",
        source="github_integration",
        payload=payload,
    ...
    log.warning(
        "github issue failed mission_id=%s code=%s",
        mission_id,
        result.error_code,
    )
