...
| **Gmail — governed create draft + send draft** | **Implemented** (narrow) | `POST .../gmail/create-draft`, `POST .../gmail/send-draft`, `app/services/gmail_draft_workflow.py`, `app/integrations/gmail_adapter.py` | Approval-gated (`gmail_create_draft` / `gmail_send_draft`, risk **red**); OAuth on control plane (`JARVIS_GMAIL_*`); send path is **existing draft only** (`drafts.send`); token needs **`gmail.send`** for send | `docs/INTEGRATIONS_GMAIL.md` |
