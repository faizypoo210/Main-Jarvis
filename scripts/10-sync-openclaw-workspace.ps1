#Requires -Version 5.1
<#
.SYNOPSIS
  Copies approved OpenClaw workspace markdown from this repo into %USERPROFILE%\.openclaw\workspace\main\

.DESCRIPTION
  Approved files (persona / policy / tools - not mission state): SOUL.md, AGENTS.md, IDENTITY.md, USERS.md, TOOLS.md
  Source: <repo>\config\workspace\<filename>
  Never touches auth-profiles.json, openclaw.json, or any path outside workspace\main approved list.

  If a file is not present under config\workspace\, the script reports the expected path and skips (no placeholder content invented).

  Creates a timestamped backup of any destination file before overwrite.
#>
$ErrorActionPreference = 'Stop'

$JarvisRoot = Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)
$SrcDir = Join-Path $JarvisRoot 'config\workspace'
$DstDir = Join-Path $env:USERPROFILE '.openclaw\workspace\main'
$Approved = @('SOUL.md', 'AGENTS.md', 'IDENTITY.md', 'USERS.md', 'TOOLS.md')

$ts = Get-Date -Format 'yyyyMMdd-HHmmss'
$BackupDir = Join-Path $DstDir (Join-Path '.jarvis-sync-backups' "pre-sync-$ts")

function Get-FileSha256([string]$Path) {
    if (-not (Test-Path -LiteralPath $Path)) { return $null }
    return (Get-FileHash -LiteralPath $Path -Algorithm SHA256).Hash
}

Write-Host "=== JARVIS: sync OpenClaw workspace (repo -> live) ===" -ForegroundColor Cyan
Write-Host "Repo source dir: $SrcDir"
Write-Host "Live destination: $DstDir"
Write-Host "Backup folder:   $BackupDir"
Write-Host ""

if (-not (Test-Path -LiteralPath $SrcDir)) {
    throw "Source directory missing: $SrcDir"
}

if (-not (Test-Path -LiteralPath $DstDir)) {
    New-Item -ItemType Directory -Path $DstDir -Force | Out-Null
    Write-Host "[INFO] Created destination directory: $DstDir" -ForegroundColor DarkGray
}

New-Item -ItemType Directory -Path $BackupDir -Force | Out-Null

$copied = 0
$skippedMissingSrc = 0
$unchanged = 0

foreach ($name in $Approved) {
    $src = Join-Path $SrcDir $name
    $dst = Join-Path $DstDir $name

    if (-not (Test-Path -LiteralPath $src)) {
        Write-Host "[MISSING-SOURCE] $name - add file at: $src" -ForegroundColor Yellow
        $skippedMissingSrc++
        continue
    }

    $srcHash = Get-FileSha256 $src
    $dstHash = if (Test-Path -LiteralPath $dst) { Get-FileSha256 $dst } else { $null }

    if ($null -ne $dstHash -and $srcHash -eq $dstHash) {
        Write-Host "[UNCHANGED] $name (hash matches live copy)" -ForegroundColor DarkGray
        $unchanged++
        continue
    }

    if (Test-Path -LiteralPath $dst) {
        $bakPath = Join-Path $BackupDir $name
        Copy-Item -LiteralPath $dst -Destination $bakPath -Force
        Write-Host "[BACKUP] $name -> $bakPath" -ForegroundColor DarkGray
    } else {
        Write-Host "[NEW] $name (no previous file at destination)" -ForegroundColor DarkGray
    }

    Copy-Item -LiteralPath $src -Destination $dst -Force
    Write-Host "[UPDATED] $name" -ForegroundColor Green
    $copied++
}

Write-Host ""
Write-Host "Summary: updated=$copied unchanged=$unchanged missing-source=$skippedMissingSrc" -ForegroundColor Cyan
Write-Host "Secrets not touched: agents/*/auth-profiles.json, openclaw.json, config.json, etc." -ForegroundColor DarkGray

# TODO(MiniMax / cloud): Provider API keys and model auth live outside this sync. Edit manually:
#   %USERPROFILE%\.openclaw\agents\main\agent\auth-profiles.json
#   and align JARVIS_OPENCLAW_GATEWAY_MODEL / openclaw.json with OpenClaw docs.
Write-Host ""
Write-Host "TODO: Configure MiniMax (or other cloud) credentials in auth-profiles.json if not already done." -ForegroundColor Yellow

exit 0
