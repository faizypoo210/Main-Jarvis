...
|--------|---------------------------|
| Control Plane HTTP API and persistence | `services/control-plane/` (FastAPI, Alembic, PostgreSQL) — includes **`memory_items`** for operator durable memory; **approval review bundles** — `GET /api/v1/approvals/{approval_id}/bundle` (`app/services/approval_review_packet.py`, `app/schemas/approval_bundle.py`); **`approval_reminders`** for governed reminder/escalation attempts (migration `008_approval_reminders`); **Operator Inbox v1** — `GET /api/v1/operator/inbox` + optional `POST .../acknowledge|snooze|dismiss` backed by **`operator_inbox_state`** (migration `009_operator_inbox_state`; items derived from existing approvals / heartbeat / mission events / missions — not a second timeline store); **Governed Action Catalog v1** — `app/services/governed_action_catalog.py`, **`GET /api/v1/operator/action-catalog`** (launch metadata for the six GitHub/Gmail governed actions; no secrets); **`governed_action_labels.py`** — shared compact titles + humanized surfaces for inbox/activity; mission events **`approval_requested`** include **`requested_by`** / **`requested_via`**; **`approval_resolved`** include **`action_type`** / **`risk_class`** for timeline/activity labels |
...
| **Heartbeat supervision (v1)** | Periodic explicit rule checks; **deduped** open/resolved rows in `heartbeat_findings`; no chat or LLM “nudges” | `app/services/heartbeat_service.py`, `heartbeat/heartbeat.py` worker, `GET /api/v1/operator/heartbeat`, Activity category `heartbeat` | **Approval reminders v1** (env `APPROVAL_*`) runs **first** inside **`POST /api/v1/heartbeat/run`**, writes **`approval_reminders`** + optional SMS; adds **`approval_escalation_pending`** findings when escalation SMS was sent and approval still pending. **Not** live OS process monitoring; **`stale_worker`** uses **`workers.last_heartbeat_at`** from **`POST /api/v1/workers/heartbeat`** (direct truth when workers register); threshold `HEARTBEAT_WORKER_STALE_MINUTES`. Coordinator/executor/voice/heartbeat worker use `shared/worker_registry_client.py`. **Cost guardrails v1:** rolling-window thresholds on **`cost_events`** (direct USD, estimated USD, unknown count, optional provider concentration) emit **`cost_*`** findings with stable dedupe keys; **`GET /api/v1/operator/cost-guardrails`** exposes config + window metrics + open/recent resolved rows — **no** invented budgets. |
| **Cost events (v1)** | Persisted rows with **`cost_status`** (`direct` / `estimated` / `unknown` / `not_applicable`), optional **`source_receipt_id`**, tokens/meta when present | `app/services/cost_event_service.py` (on `POST /api/v1/receipts`), `GET /api/v1/operator/cost-events` | **Not** a billing platform; GitHub/Gmail receipts are typically **`not_applicable`**; OpenClaw without USD/tokens in payload is **`unknown`**. |
| **Operator UI** | Browse missions, act on approvals (including **packet-based review** on `/approvals`), **`/inbox`** triage queue (hand-off to missions/approvals/cost/system), mission detail, live updates | `services/command-center/` | N/A |
| **Command intake** | Create missions from text + surface metadata | `POST /api/v1/commands`, `app/schemas/commands.py` | Command Center **Overview** composer + **shell quick command** (any route) use the same API path; shell focus + thread hydration are UI concerns |
| **Redis coordination** | Stream-based handoff; payloads carry **routing** metadata; receipts add **lane_truth** (reconciled routing + OpenClaw model lane) | `coordinator/coordinator.py`, `executor/executor.py`, `shared/routing.py`, `shared/lane_truth.py`, `docs/MODEL_LANES.md` | Redis persistence / Docker volume; **no** separate local-fast **mission** worker — executor is OpenClaw-only |
...
| **Voice path** | STT/TTS and forwarding toward control plane / streams; **direct Ollama** acks do not populate mission `routing_decided` or executor `lane_truth`; **Voice inbox triage v1** (`inbox_voice.py`) reads **`GET /api/v1/operator/inbox`** and optional **`POST .../acknowledge|snooze|dismiss`** with explicit phrases only (ephemeral focus per WebSocket); **Voice briefing v1** reads operator truth only; **Voice governed action requests v1** (`governed_action_voice.py`) creates **pending approvals** via the same mission integration POSTs as the Command Center — **not** direct GitHub/Gmail execution; reuses **`GET /api/v1/operator/action-catalog`** for field order and prompts when the catalog loads (narrow regex intents unchanged) | `voice/`, `voice/inbox_voice.py`, `voice/briefing_voice.py`, `voice/governed_action_voice.py` | Whisper/GPU env, local `.env` beside `server.py`; `CONTROL_PLANE_API_KEY` for inbox triage + integration POSTs + catalog; lane vocabulary: `docs/MODEL_LANES.md` |
...
| Stack bring-up / readiness (Windows) | `jarvis.ps1` initiates bring-up and prints per-surface honesty; **`scripts/07-verify-jarvis-stack.ps1`** is the stricter gate (containers + `/health` + gateway TCP + Command Center HTTP; **not** full process supervision) |
...
| SMS approval notify + reply (optional Twilio) | `approval_sms_tokens` table; outbound when `JARVIS_SMS_APPROVALS_ENABLED` + Twilio env set; inbound `POST /api/v1/integrations/sms/inbound` — explicit `APPROVE` / `DENY` / `READ` + code only; see `docs/SMS_APPROVALS.md` |
...
| End-of-day handoff snapshot | `scripts/19-day-wrap-snapshot.ps1` writes `docs/reports/day-wrap-*.md` — multi-surface readiness (governed catalog + inbox/workers/cost + optional Phase 8 + Command Center build + workspace smoke). **Pass/fail/skip** are explicit; **SKIP** when control plane is unreachable or optional steps are disabled; does not assert live coordinator/E2E. |
| Governed action catalog smoke | `scripts/19-smoke-governed-action-catalog.ps1` — `GET /api/v1/operator/action-catalog` and six `approval_action_type` rows |
| Operator inbox / heartbeat / workers / cost reads (smoke) | `scripts/19-smoke-operator-surfaces.ps1` — `GET` inbox, heartbeat, workers, cost-guardrails, cost-events |
...
| Worker registry (direct) | `POST /api/v1/workers/register`, `POST /api/v1/workers/heartbeat` with `x-api-key`; `GET /api/v1/operator/workers`; workers: `CONTROL_PLANE_URL`, `CONTROL_PLANE_API_KEY`, optional `JARVIS_WORKER_INSTANCE_ID`, `JARVIS_WORKER_HEARTBEAT_INTERVAL_SEC` |
| Cost events (direct store) | `GET /api/v1/operator/cost-events` (filters: `provider`, `cost_status`, `mission_id`); rows created when receipts are recorded (`cost_events` migration `006_cost_events_v1`) |
| Cost guardrails snapshot | `GET /api/v1/operator/cost-guardrails` — env thresholds, window rollups from `cost_events`, breach flags, open + recent resolved **`cost_*`** heartbeat findings |
...
- **Command Center** exposes **Integrations**, **Workers**, **Cost & Usage**, **System Health**, and **Activity** as **API-backed** operator pages. Integrations shows **readiness and honesty signals** from `GET /api/v1/operator/integrations` (DB rows + safe machine/repo probes); it does **not** perform OAuth or store vendor tokens. **Governed GitHub/Gmail workflows** are invoked from **Mission detail → Request a governed action** (typed forms calling the same `POST /api/v1/missions/{id}/integrations/...` routes as scripts/API); see `docs/INTEGRATIONS_GITHUB.md` / `docs/INTEGRATIONS_GMAIL.md` for server contracts — the Integrations page is readiness only, not the launcher.
