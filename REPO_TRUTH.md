...
| **Voice path** | STT/TTS and forwarding toward control plane / streams; **direct Ollama** acks do not populate mission `routing_decided` or executor `lane_truth`; **Voice briefing v1** reads operator truth only; **Voice governed action requests v1** (`governed_action_voice.py`) creates **pending approvals** via the same mission integration POSTs as the Command Center — **not** direct GitHub/Gmail execution | `voice/`, `voice/briefing_voice.py`, `voice/governed_action_voice.py` | Whisper/GPU env, local `.env` beside `server.py`; `CONTROL_PLANE_API_KEY` for integration POSTs; lane vocabulary: `docs/MODEL_LANES.md` |
