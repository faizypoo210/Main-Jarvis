...
| **Voice path** | STT/TTS and forwarding toward control plane / streams; **direct Ollama** acks do not populate mission `routing_decided` or executor `lane_truth`; **Voice briefing v1** reads operator truth only (missions, heartbeat, workers, activity, etc.) — no invented system state | `voice/`, `voice/briefing_voice.py` | Whisper/GPU env, local `.env` beside `server.py`; lane vocabulary: `docs/MODEL_LANES.md` |
