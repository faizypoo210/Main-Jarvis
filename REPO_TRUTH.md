...
|--------|---------------------------|
| Control Plane HTTP API and persistence | `services/control-plane/` (FastAPI, Alembic, PostgreSQL) — includes **`memory_items`** for operator durable memory; **approval review bundles** — `GET /api/v1/approvals/{approval_id}/bundle` (`app/services/approval_review_packet.py`, `app/schemas/approval_bundle.py`); **`approval_reminders`** for governed reminder/escalation attempts (migration `008_approval_reminders`) |
